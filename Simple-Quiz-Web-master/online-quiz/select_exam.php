...
$sql = "SELECT * FROM exam_category WHERE status = 1";
...
?>
    ...
    for ($i=0;$row = mysqli_fetch_array($result);$i++) {
        $category_name = $row['describe'];
        $exam_logo = $row['logo'];
        $cate_id = $row['id'];

        ?>

                <button class="my-btn" onclick="set_exam_type_session('<?php echo $cate_id; ?>')">
                    <br>
...
<script type="text/javascript">
function set_exam_type_session(exam_category_id) {
    ...
    xmlhttp.open("GET", "forajax/set_exam_type_session.php?exam_category=" + exam_category_id, true);
