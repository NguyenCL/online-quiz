...
if (isset($_POST['submit'])) { // sử dụng hàm isset($_POST['submit']) kiểm tra xem biểu mẫu đã được gửi đi chưa
    ...
    // và lấy thông tin username và password được gửi từ biểu mẫu
    $username = mysqli_real_escape_string($link, $_POST['username']);
    $pass = md5($_POST['password']); // md5() để mã hóa password

    // truy vấn SQL
    $select = " SELECT * FROM registration WHERE username = '$username' && password = '$pass' ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $_SESSION['username'] = $row['username']; // lưu tên quản trị viên vào session với giá trị của trường name
            $_SESSION['user_role'] = $row['role']; // lưu quyền của account vào session
            $_SESSION['user_id'] = $row['id'];

            // kiểm tra user login có quyền admin hoặc teacher 
            if (in_array($row['role'], array('admin', 'teacher'))) { 
                // ----- user login là admin hoặc teacher ----- => chuyển hướng đến trang admin
                header('location:../online-quiz/admin/exam_category.php'); 
                exit(); // dừng toàn bộ quá trình và chuyển hướng đến trang khác
            } else {
                // ----- user login là student -----
                header('location:../online-quiz/select_exam.php'); 
            }
        }
...
<head>
    ...
    <!-- Tên web -->
    <title>Đăng nhập admin || teacher</title>
